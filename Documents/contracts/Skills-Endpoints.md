...
- Users can update their own career interests
- Mentors can update their own expertise tags (via their user account)
...
## 6. Update User Career Interests

**Endpoint:** `PATCH /api/users/me/career-interests`  
**Requires:** `Authorization: Bearer {token}`  
...
**Field Requirements:**
- `skillIds` (required): Array of skill IDs
- Array can be empty to clear all career interests
- Skill IDs must exist and be active
...
- **400 Validation Error:**
  ```json
  {
    ...
    "message": "Validation failed",
    "errors": {
      "SkillIds": ["One or more skill IDs are invalid or inactive"]
    },
    "statusCode": 400
  }
  ```
...
- Remove all existing UserSkills entries for this user
- Insert new UserSkills entries for provided skill IDs
- Return updated list of user's career interests with full skill details

---

## 7. Update Mentor Expertise Tags

**Endpoint:** `PATCH /api/mentors/{mentorId}/expertise-tags`  
**Requires:** `Authorization: Bearer {token}`  
**Roles:** Mentor (own profile) or Admin

**Path Parameters:**
- `mentorId` (integer): Mentor ID to update
...
**Field Requirements:**
- `skillIds` (required): Array of skill IDs
- Array can be empty to clear all expertise tags
- Skill IDs must exist and be active
...
- **403 Forbidden:**
  ...
  {
    "success": false,
    "message": "You can only update your own expertise tags",
    "statusCode": 403
  }
  ```
...
**Backend Behavior:**
- Verify user is the mentor owner (via Mentor.UserId) or has Admin role
- Fetch mentor record to get UserId
- Validate all skill IDs exist and are active
- Remove all existing UserSkills entries for the mentor's UserId
- Insert new UserSkills entries for provided skill IDs
- Return updated list of mentor's expertise tags with full skill details
