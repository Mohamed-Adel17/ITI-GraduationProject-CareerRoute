...
export class SignalRNotificationService implements OnDestroy {
      ...
      this.hubConnection = new signalR.HubConnectionBuilder()
        ...
        .withAutomaticReconnect({
          nextRetryDelayInMilliseconds: (retryContext: any) => {
            const delays = [0, 2000, 5000, 10000];
            if (retryContext.previousRetryCount < delays.length) return delays[retryContext.previousRetryCount];
            return null;
          }
        })
        .configureLogging(signalR.LogLevel.Information)
  ...
  private setupNotificationHandlers(): void {
    ...
    this.hubConnection.on('ReceiveNotification', (notification: unknown) => {
      ...
      if (this.processedNotificationIds.has(parsedNotification.id)) return;

      this.processedNotificationIds.add(parsedNotification.id);
      setTimeout(() => this.processedNotificationIds.delete(parsedNotification.id), this.DEDUP_WINDOW_MS);
  ...
  private parseNotification(data: unknown): NotificationDto | null {
    ...
    if (typeof id !== 'string' || !type || typeof title !== 'string' || typeof message !== 'string' || typeof isRead !== 'boolean' || typeof createdAt !== 'string') {
      return null;
    }
  ...
  private convertNotificationType(typeRaw: unknown): NotificationType | null {
    ...
    if (typeof typeRaw === 'number') return numericTypeMap[typeRaw] ?? null;
    return null;
  }
  ...
  setUnreadCount(count: number): void {
    this.unreadCountSubject.next(typeof count === 'number' && !isNaN(count) ? count : 0);
  }
  ...
  waitForConnection(): Observable<ConnectionState> {
    return this.connectionState$.pipe(filter(state => state === ConnectionState.Connected), take(1));
  }
}